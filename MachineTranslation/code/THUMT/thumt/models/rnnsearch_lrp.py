...
def _decoder(cell, inputs, memory, sequence_length, initial_state, w_x_enc, 
    ...
    with tf.variable_scope(scope or "decoder", dtype=dtype):
        ...
        def loop_func(t, out_ta, att_ta, val_ta, state, cache_key, wxh_ta, 
            ...
            att = tf.Print(att, [tf.shape(att), tf.shape(w_x_enc), tf.shape(context),tf.shape(memory)], summarize=100)
            ...
            return t + 1, out_ta, att_ta, val_ta, new_state, cache_key, wxh_ta, \
                   wxc_ta, w_x_h_new
...
def model_graph(features, labels, params):
    ...
    R_x_maxout = tf.matmul(w_x_maxout, tf.expand_dims(maxhid, -1))
    R_x_maxout = tf.squeeze(R_x_maxout, -1)
    R_x_maxout = normalize(R_x_maxout, True)

    rlv_info["maxout"] = R_x_maxout
    rlv_info["result"] = R_x_maxout
