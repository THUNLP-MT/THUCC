...
def model_graph(features, labels, mode, params):
    ...
    wr_logit_decoder = logits_elewise_true / lrp.stabilize(tf.expand_dims(logits_true, -1), params.stab)
    w_x_true = w_x_dec * tf.expand_dims(wr_logit_decoder, 1)
    w_x_true = tf.reduce_sum(w_x_true, -1)
    decoder_output = tf.Print(decoder_output, [tf.shape(decoder_output), tf.shape(weights), tf.shape(labels), tf.shape(weights_true), tf.shape(w_x_dec), tf.shape(wr_logit_decoder)], summarize=100)
    ...
    rlv_info = {}
    '''
    R_x_dec = w_x_dec*tf.expand_dims(decoder_out["outputs"], 1)
    R_x_dec = tf.reduce_sum(R_x_dec, -1)
    R_x_dec = tf.transpose(R_x_dec, [0,2,1])
    rlv_info["raw"] = R_x_dec
    R_x_dec = normalize(R_x_dec, True)
    rlv_info["result"] = R_x_dec
    '''
